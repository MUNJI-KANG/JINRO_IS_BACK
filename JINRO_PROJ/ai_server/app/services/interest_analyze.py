...
# 3. 5프레임 단위 영상 분석 함수 (frame_skip 변수 추가)
# def analyze_video_with_face_crop(video_path, model, transforms, class_names, device, face_detector, frame_skip=5):
...
#     print(f"⚡ {frame_skip} 프레임 간격으로 분석을 진행하여 속도를 최적화합니다.")
...
#     # 🌟 핵심: range를 0부터 시작하되, frame_skip(5) 간격으로 건너뛰며 반복문 실행
#     for frame_idx in tqdm(range(0, total_frames, frame_skip), desc="영상 분석 중"):
        
#         # 🌟 핵심: OpenCV에게 해당 프레임 번호로 바로 건너뛰라고 지시
...
#     overall_stats = {
#         'Total_Frames_Analyzed': len(frame_data_list), # 실제로 분석한 프레임 수
